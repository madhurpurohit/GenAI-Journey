...
async function main() {
  ...
  try {
    ...
    console.log(
      `✅ Gemini initialized (model: ${process.env.GEMINI_MODEL || "gemini-2.5-flash"})`,
    );
  } catch (error) {
  ...
  if (resumeIndex !== -1) {
    ...
    console.log(
      `  🧵 Thread ID: ${threadId}  (save this to resume if needed)\n`,
    );
    console.log("─".repeat(60));
  }
  ...
  try {
    ...
    if (finalState.sandboxId) {
      ...
      console.log(
        `  🏥 Healthy: ${finalState.sandboxHealthy ? "✅ Yes" : "❌ No"}`,
      );
